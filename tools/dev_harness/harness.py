...
def execute(argv, cwd, timeout=180, env=None):
    ...
    with subprocess.Popen(argv, cwd=cwd, env=env, stdout=subprocess.PIPE,
        ...
        try:
            ...
            code = 124
    return dict(command=argv, exit_code=code, output=output.decode(errors='replace'),
                timestamp=started, duration_seconds=round(time.time()-started, 3))
...
def snapshot(repo):
    ...
    for name in sorted(set(names['output'].split('\0')) - {''}):
        ...
        if path.is_symlink():
            ...
            data = b'<missing>'
        result[name] = hashlib.sha256(data + str(stat.S_IMODE(path.lstat().st_mode) if path.exists() else 0).encode()).hexdigest()
...
def validate(c):
    ...
    for field in ['unchanged', 'acceptance', 'negative_cases', 'files', 'checks', 'gates', 'dependencies']:
    ...
    for name in c['files']:
        ...
        if name in ['.git', '.gitignore', 'opencode.json', 'opencode.jsonc', 'AGENTS.md'] or name.startswith(('.git/', '.opencode/', '.omo/', '.harness/', 'tools/dev_harness/')):
            raise ValueError('Harness/config edits require senior review')
    for check in c['checks']:
        if not isinstance(check.get('argv'), list) or not check['argv'] or not all(isinstance(x, str) for x in check['argv']):
...
def agent(role, prompt, repo, c, routing):
    ...
    permission = {'*': 'deny', 'read': {'*': 'allow', '*.env*': 'deny', '*credentials*': 'deny', '*token.json': 'deny', '*.pem': 'deny', '*.key': 'deny', '*lifeos_bootstrap*': 'deny'}, 'glob': 'allow', 'grep': 'deny',
                  'external_directory': 'deny', 'edit': 'deny'}
    if role in ['builder', 'debugger']:
        permission['edit'] = {'*': 'deny', **{str(repo / p): 'allow' for p in c['files']},
                              **{p: 'allow' for p in c['files']}}
    config = {'model': model, 'small_model': model, 'share': 'disabled',
              'permission': permission, 'enabled_providers': ['opencode'],
              'agent': {'harness-worker': {'mode': 'primary', 'model': model,
                         'steps': 24, 'permission': permission,
    ...
    return execute(['opencode', 'run', '--pure', '--agent', 'harness-worker',
                    '--model', model, '--format', 'json', prompt], repo,
                   timeout=routing.get('timeout_seconds', 240), env=env)
    ...
    for index, check in enumerate(c['checks']):
        ...
        if check['kind'] == 'pytest':
            try:
                ...
                r['counts'] = counts
                r['passed'] &= (counts['tests'] > 0 and counts['failures'] == 0
...
def run(c, repo, out, routing, invoke=agent):
    ...
    state = dict(run_id=out.name, repository=str(repo), status='NOT_STARTED', attempts=[],
                 routing=routing, contract=c, created=time.time())
    ...
    def save(status, reason=''):
        ...
        with (out / 'events.jsonl').open('a') as events:
            events.write(json.dumps({'timestamp': time.time(), 'status': status, 'reason': reason}) + '\n')
        diff = execute(['git', 'diff', 'HEAD', '--'], repo)['output']
        (out / 'diff.patch').write_text(diff)
        packet = dict(task=c, status=status, reason=reason, source=state['source'],
                      git_status=execute(['git', 'status', '--short'], repo)['output'],
                      attempts=state['attempts'], evidence=str(out),
    ...
    for dep in c['dependencies']:
        ...
        if d['status'] != 'VERIFIED' or d['repository'] != str(repo) or d['source'] != snapshot(repo):
            return save('BLOCKED', 'Prerequisite is not VERIFIED at current repository state: ' + dep)
    if c.get('risk') in ['high', 'security']:
        return save('BLOCKED', 'Senior review required before implementation; no paid model invoked')
    ...
    for attempt in range(1, 3):
        ...
        save('IN_PROGRESS')
        result = invoke(role, 'Implement only the contract files. No shell, delegation, or external '
                        'services. The harness runs checks. Report partial work honestly.\nCONTRACT\n' +
                        context + '\nPRIOR FAILURE\n' + failure[-18000:], repo, c, routing)
        ...
        diff = execute(['git', 'diff', 'HEAD', '--'], repo)['output']
        review = invoke('verifier', 'Independently read every contract file; inspect missing '
        ...
        try:
            verdict = json.loads(model_text(review).strip().removeprefix('```json').removesuffix('```'))
        ...
        passed = (all(e['passed'] for e in evidence) and review['exit_code'] == 0
                  and verdict.get('verdict') == 'VERIFIED'
                  and all(verdict.get('criteria', {}).get(a) is True for a in c['acceptance']))
        if passed:
            if c['gates']:
                return save('PHYSICAL_VERIFICATION_REQUIRED',
                            'READY_FOR_PHYSICAL_VERIFICATION: ' + ', '.join(c['gates']))
            return save('VERIFIED')
        failure = json.dumps({'checks': evidence, 'review': verdict, 'prior_approach': item['builder_report']})
    ...
    return save('BLOCKED', 'Two verification failures. STOP; senior escalation packet: handoff.json')
...
def main():
    ...
    if args.action == 'status':
        ...
        if state['source'] != snapshot(Path(state['repository'])):
            state.update(status='IMPLEMENTED_UNVERIFIED', reason='Repository changed; evidence is stale')
    else:
        routing = json.loads((HOME / 'routing.json').read_text())
        out = Path.home() / '.local/state/lifeos-harness' / (time.strftime('%Y%m%d-%H%M%S-') + uuid.uuid4().hex[:8])
        try:
